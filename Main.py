...
AUTHORIZED_ROLE_ID = 1512023301432541291
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await send_error(
                interaction,
                "Role Error",
                "I could not assign the roles. Please place my bot role above the trainee, authorised, and department roles."
            )
            return
...
@bot.tree.command(name="deleteregistration", description="Completely delete a trainee registration and allow re-registration.")
async def deleteregistration(
    ...
    steps = [
        ...
        "Checking trainer permissions",
        "Removing academy roles",
        "Deleting registration records",
        "Verifying deletion",
        "Logging deletion",
        "Registration deleted successfully"
    ]
    progress_message = await start_progress(
        interaction,
    ...
    if not isinstance(interaction.user, discord.Member) or not is_trainer(interaction.user):
        await fail_progress(
            ...
            steps,
            1,
            "Only Institute Trainers may delete registrations."
        )
        return

    # Remove trainee and department roles first. Stale roles previously caused
    # /register to incorrectly report that the user was already registered.
    await update_progress(
        progress_message,
        f"{I17} Delete Registration Progress",
        steps,
    ...
    if roles_to_remove:
        try:
            await user.remove_roles(
                *roles_to_remove,
                reason=f"Academy registration deleted by {interaction.user}"
            )
        except discord.Forbidden:
            await fail_progress(
                progress_message,
                ...
                steps,
                2,
                "I could not remove the trainee roles. Move the bot role above all academy trainee and department roles, then try again."
            )
            return
            ...
            await fail_progress(
                ...
                steps,
                2,
                f"Discord could not remove the trainee roles: `{exc}`"
            )
            return
    ...
    try:
        # Delete every trainee record which can prevent a fresh registration.
        ...
        await fail_progress(
            ...
            steps,
            3,
            f"The database could not delete the registration: `{exc}`"
        )
        return
    ...
    cursor.execute("SELECT 1 FROM pending_registrations WHERE discord_id = ?", (user.id,))
    pending_still_exists = cursor.fetchone() is not None
    cursor.execute("SELECT 1 FROM registrations WHERE discord_id = ?", (user.id,))
    approved_still_exists = cursor.fetchone() is not None

    if pending_still_exists or approved_still_exists:
        await fail_progress(
            progress_message,
            f"{I17} Delete Registration Progress",
            steps,
            4,
            "The registration could not be fully removed from the database."
        )
        return
    ...
    removed_role_names = ", ".join(role.name for role in roles_to_remove) or "None"
    ...
    await send_log(
        ...
        f"Roles Removed: {removed_role_names}\n"
        f"Pending Records Deleted: {pending_deleted}\n"
        f"Approved Records Deleted: {approved_deleted}\n"
        f"Training Logs Deleted: {training_deleted}\n"
        f"Examination Logs Deleted: {exam_deleted}\n"
        f"Re-registration: Allowed"
    )
    ...
    await finish_progress(
        ...
        f"{I17} Delete Registration Progress",
        steps,
        f"{user.mention}'s academy registration was completely deleted. They may now use `/register` again."
    )
